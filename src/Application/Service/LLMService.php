...
class LLMService
{
    ...
    private function buildRAGPrompt(string $question, string $context): string
    {
        ...
        if ($language === 'en') {
            ...
            $systemPrompt = "Ты - русскоязычный AI-ассистент. Отвечай ТОЛЬКО на русском языке (кириллица). " .
                "НЕ используй китайский или английский язык. " .
                "ВАЖНО: Используй ТОЛЬКО информацию из контекста. НЕ придумывай имена или факты.";
            $prompt = "{$systemPrompt}\n\n";
            $prompt .= "Контекст:\n{$context}\n\n";
            $prompt .= "Вопрос: {$question}\n\n";
            $prompt .= "Ответ (начни с точного числа, если спрашивают количество):";
        }
